...
const ProductDetail = (props) => {
  ...
  const deleteThisProduct = (id) => {
    ...
    ).then(() => window.alert("Deleted Success :)"));
  };
  return (
    <div className="font-kanit md:text-lg lg:text-xl xl:text-2xl xl:flex xl:flex-col xl:justify-center xl:items-center lg:pb-20">
      {product.Brands ? (
        <div className="text-2xl p-8 md:text-4xl xl:text-5xl xl:p-10">
        ...
        <div className="card shadow-xl rounded-3xl md:w-96 lg:w-112">
          {product.Image ? (
            <img
              ...
              alt="NOT LOADED"
              src={`${process.env.REACT_APP_API_URL}/api/download/image/${product.Image}`}
            />
          ) : null}
          <Textbox className="text-left p-8 space-y-2 rounded-b-3xl relative">
            ...
            <li className="text-xl md:text-2xl lg:text-3xl xl:text-4xl">
              {product.ProdName}
            </li>
